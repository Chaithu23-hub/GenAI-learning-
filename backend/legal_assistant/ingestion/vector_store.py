...
# [Vector databases (HNSW)] [Metadata filtering] [Qdrant / Chroma / pgvector]
COLLECTION_NAME = "legal_docs"
TOKEN_RE = re.compile(r"[a-z0-9]+(?:[-'][a-z0-9]+)*")


def document_type_for(filename):
    """Derive document_type metadata from filename convention.
    Files whose stem starts with 'amendment' are tagged as amendments."""
...
def ingest_documents(docs_dir=None, client=None):
    """Chunk, embed, and upsert all markdown documents in docs_dir into Chroma."""
...
def query_store(query, k=config.TOP_K_CANDIDATES, where=None, client=None):
    """Dense similarity search against the Chroma vector store."""
    collection = get_collection(client)
...
def keyword_store(query, k=config.TOP_K_CANDIDATES, where=None, client=None):
    """BM25-style lexical scan over the full collection."""
    collection = get_collection(client)
