import re
from dataclasses import dataclass

from .. import config

# [Chunking strategies] [Tokens & tokenization]
# Tokens are approximated by whitespace splits to stay CPU-friendly and
# dependency-free (no tiktoken required).
HEADING_RE = re.compile(r"^(#{1,6})\s+(.+?)\s*$")


@dataclass
class Chunk:
    text: str
    heading: str   # section heading for the chunk, or "Preamble"
    index: int     # 0-based position within the document


def split_into_sections(text):
    """Split a markdown document into (heading, body) pairs."""
    sections = []
    heading, lines = "Preamble", []
    for line in text.splitlines():
        match = HEADING_RE.match(line)
        if match:
            body = "\n".join(lines).strip()
            if body:
                sections.append((heading, body))
            heading, lines = match.group(2).strip(), []
        else:
            lines.append(line)
    body = "\n".join(lines).strip()
    if body:
        sections.append((heading, body))
    return sections


def chunk_document(text, chunk_size=config.CHUNK_SIZE_TOKENS, overlap=config.CHUNK_OVERLAP_TOKENS):
    """Chunk a document into overlapping windows of approximately chunk_size tokens."""
    chunks = []
    for heading, body in split_into_sections(text):
        words = body.split()
        if len(words) <= chunk_size:
            chunks.append(Chunk(text=body, heading=heading, index=len(chunks)))
            continue
        step = chunk_size - overlap
        for start in range(0, len(words), step):
            window = words[start:start + chunk_size]
            chunks.append(Chunk(text=" ".join(window), heading=heading, index=len(chunks)))
            if start + chunk_size >= len(words):
                break
    return chunks
