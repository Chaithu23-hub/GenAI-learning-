...
# [Grounded generation & citations] [Tool calling] [Validation & retry]
# [Temperature & sampling] [Prompt anatomy] [Zero-shot vs few-shot]
# [Cost per token] [Context window] [Parallel tool calls] [Why RAG]
...
# Legal concept completeness requirements for confidence downgrading
_CONCEPT_REQUIREMENTS = {
    "takings clause": ["public", "compensation"],
    "bill of rights": ["liberties", "government"],
    "amendments": ["amendment", "ratified"],
    "sovereignty": ["states", "immunity"],
    "double jeopardy": ["twice", "same crime"],
    "jury trial": ["criminal", "civil"],
    "amendment gaps": ["year", "decades"],
    "anti-federalist": ["feared", "government", "liberty"],
}


def _log_usage(response):
    """Log token usage to stderr so cost per request can be tracked."""
...
def _check_answer_completeness(question: str, answer_dict: dict) -> dict:
    """Check if the answer covers all required legal concepts for the question type."""
...
def _apply_completeness_adjustment(payload: dict, question: str) -> dict:
    """Downgrade confidence if the answer is missing required legal concepts."""
...
def detect_llm_model():
    """Probe the configured LLM endpoint for available models; return the best match."""
...
class ExtractiveGenerator:
    ...
    def generate(self, query, where=None):
        ...
        # Every sentence in the answer maps to a source below.
        parts, sources = [], []
        for c in relevant:
        ...
        return _apply_completeness_adjustment(payload, query)


def _verify_sources(payload):
    """Verify that all cited chunk_ids exist in the vector store and excerpts match."""
...
class LLMGenerator:
    """Google Gemini-backed generator with tool calling, JSON validation, and retry."""
    ...
    def _generate_llm(self, query, where=None):
        ...
        for _ in range(self.MAX_TOOL_ROUNDS):
            ...
            for call in message.tool_calls:
                ...
                # Restrict search to contracts or amendments via the tool argument.
                messages.append({
                    "role": "tool",
        ...
        # Validate the JSON shape and verify citations; retry once if either fails.
...
def get_generator(backend=None):
    """Factory: return the appropriate generator based on backend flag and env."""
    if backend == "auto":
        backend = None
