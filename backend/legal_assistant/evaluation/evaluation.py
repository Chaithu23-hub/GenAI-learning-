...
# Labels are tied to the headings in the bundled corpus.
EVALUATION_CASES = (
...
def hit_rate_at_k(cases=EVALUATION_CASES, k=3, hybrid=config.HYBRID_ENABLED, client=None):
    """Compute Hit Rate @ k: fraction of questions where a labeled chunk appears in top-k."""
    hits = 0
    for case in cases:
...
def compare_retrieval(cases=EVALUATION_CASES, k=3, client=None):
    """Compare dense-only vs hybrid retrieval hit rates."""
    return {
        "k": k,
        "questions": len(cases),
