...
def test_before_and_after_tool_discovery():
    ...
    assert after["clause-search"] == before["clause-search"]
...
def test_clause_error_is_recoverable():
    async def call():
        async with MCPHost() as host:
            return await host.call(
                "clause-search",
                "get_contract_clause",
                {"contract_id": "MSA-2021-0142", "clause_number": "12.4"},
            )

    result = asyncio.run(call())
    error = result["content"][0]["text"]
    assert "clauses run 1-11" in error
    assert "2023-06-01" in error
